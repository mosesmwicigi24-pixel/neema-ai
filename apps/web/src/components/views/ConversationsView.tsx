...
export function ConversationsView({
    ...
    const listPanel = (
        <div
            className="flex flex-col bg-white border-r border-stone-100 flex-shrink-0 overflow-hidden"
        ...
        >
            ...
            <div className="px-3 pt-3 pb-2 space-y-2">
                <div className="relative">
                    ...
                    <input
                        ...
                        placeholder="Search…"
                        className="w-full h-9 pl-9 pr-3 text-sm bg-stone-50 border border-stone-200 rounded-lg text-stone-800 placeholder-stone-300 focus:outline-none focus:ring-2 focus:ring-green-600 focus:border-transparent transition-all"
                        style={{ fontSize: 16 }}
                    />
                </div>
                ...
                <div className="flex items-center gap-1.5">
                    <button
                        ...
                        className={`flex items-center gap-1 px-2.5 h-7 rounded-md text-xs font-medium transition-colors ${showFilters ? "bg-green-50 text-green-800 border border-green-200" : "bg-stone-100 text-stone-500 border border-transparent hover:bg-stone-200"}`}
                    >
                        <svg
                            className="w-3 h-3"
                            fill="none"
                    ...
                    {(["all", "open", "closed"] as const).map((s) => (
                        <button
                            ...
                            className={`px-2.5 h-7 rounded-md text-xs font-medium capitalize transition-colors ${statusFilter === s ? "bg-stone-800 text-white" : "bg-stone-100 text-stone-500 hover:bg-stone-200"}`}
                        >
                            {s}
                        </button>
                    ))}
                        ...
                        {(["all", "human", "ai", "paused"] as const).map(
                            (m) => (
                                <button
                                    ...
                                    className={`px-2 h-6 rounded text-[11px] font-medium capitalize transition-colors ${interceptFilter === m ? "bg-green-100 text-green-900 border border-green-500" : "bg-stone-100 text-stone-400 hover:bg-stone-200"}`}
                                >
                                    {m}
                                </button>
                            ),
                ...
                {CHANNEL_TABS.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-1 px-3 py-2.5 text-xs font-semibold whitespace-nowrap flex-shrink-0 border-b-2 transition-all ${isActive ? "border-green-700 text-green-800" : "border-transparent text-stone-400 hover:text-stone-600 hover:border-stone-300"}`}
                        >
                            {tab.id !== "all" && (
                                <span
                                    style={{
                            ...
                            {tab.label}
                            {count > 0 && (
                                <span className="bg-green-700 text-white rounded-full text-[10px] font-bold min-w-[16px] h-4 flex items-center justify-center px-1">
                                    {count}
                                </span>
                            )}
                        </button>
            ...
            <div className="flex-1 overflow-y-auto">
                {filteredConvs.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-40 text-sm text-stone-300 gap-2">
                        <svg
                    ...
                    filteredConvs.map((conv) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-3 py-3 border-b border-stone-50 hover:bg-stone-50 transition-colors text-left touch-manipulation ${isActive ? "bg-green-50 border-l-2 border-l-green-700" : "border-l-2 border-l-transparent"} ${conv.status === "closed" ? "opacity-50" : ""}`}
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-1 mb-0.5">
                                        <span
                                            className={`text-sm font-semibold truncate ${isActive ? "text-green-900" : "text-stone-800"}`}
                                        >
                                            {conv.name}
                                        </span>
                                    ...
                                    <div className="flex items-center justify-between gap-1">
                                        ...
                                        <div className="flex items-center gap-1 flex-shrink-0">
                                            ...
                                            />
                                            {conv.unread > 0 && (
                                                <span className="bg-green-700 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                                                    {conv.unread}
                                                </span>
                                            )}
                                        </div>
    ...
    const threadPanel = (
        <div className="flex-1 flex flex-col overflow-hidden bg-stone-50">
            {activeConv ? (
                <>
                    {/* Header */}
                    <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-stone-100 flex-shrink-0">
                        {isMobile && (
                            <button
                                onClick={() => setMobilePanel("list")}
                                className="text-green-700 p-1 -ml-1 touch-manipulation"
                            >
                                <svg
                                    className="w-5 h-5"
                                    fill="none"
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <span
                                className={`text-xs px-2 py-0.5 rounded-md font-medium ${activeConv.status === "open" ? "bg-emerald-50 text-emerald-600 border border-emerald-200" : "bg-stone-100 text-stone-400 border border-stone-200"}`}
                            >
                                {activeConv.status}
                            </span>
                        </div>
                    ...
                    {/* Action toolbar */}
                    <div className="flex items-center gap-1.5 px-4 py-2 bg-white border-b border-stone-100 flex-shrink-0 overflow-x-auto scrollbar-none">
                    ...
                    <div className="flex-1 overflow-y-auto px-4 py-5 space-y-4">
                        {activeMessages.map((msg) => {
                            ...
                            return (
                                ...
                                >
                                    <span
                                        className={`text-[11px] mb-1 font-medium ${senderColor[msg.sender] ?? "text-stone-400"}`}
                                    >
                        ...
                        {draftVisible &&
                            activeConv.intercept_mode === "human" && (
                                <div className="flex flex-col items-end">
                                    ...
                                    <div className="max-w-[78%] px-3.5 py-3 rounded-2xl rounded-br-md bg-blue-50 border border-dashed border-blue-200 text-stone-800 text-sm leading-relaxed">
                                        {AI_DRAFT}
                    ...
                    {/* Composer */}
                    <div className="bg-white border-t border-stone-100 px-4 pt-3 pb-3 flex-shrink-0">
                        ...
                        <div className="flex gap-2 items-end">
                            <textarea
                                ...
                                placeholder={
                                    activeConv.intercept_mode === "human"
                                        ? `Reply via ${CHANNEL_CONFIG[activeConv.channel].label}…`
                                        : "Intercept to reply…"
                                }
                                disabled={
                                    activeConv.intercept_mode !== "human" ||
                                ...
                                rows={2}
                                className="flex-1 px-3 py-2.5 text-sm bg-stone-50 border border-stone-200 rounded-xl text-stone-800 placeholder-stone-300 focus:outline-none focus:ring-2 focus:ring-green-600 resize-none disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                                style={{ fontSize: 16 }}
                            />
                            <Btn
                                onClick={sendReply}
        ...
        <>
                    ...
                    {agents
                        ...
                        .map((agent) => (
                            <button
                                ...
                                className="w-full flex items-center gap-3 p-3 rounded-xl border border-stone-100 hover:bg-stone-50 transition-colors text-left touch-manipulation"
                            >
                                <div className="relative">
                                    <Avatar name={agent.name} size="sm" />
                                    <div
                                        className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full ring-2 ring-white ${agent.is_available ? "bg-emerald-500" : "bg-stone-400"}`}
                                    />
                                </div>
                                <div className="flex-1 min-w-0">
            ...
            >
                <p className="text-sm text-stone-500 mb-4">
                    Notes are visible to agents only and won't be sent to the
                    customer.
                </p>
                <InputField
                    label="Note"
