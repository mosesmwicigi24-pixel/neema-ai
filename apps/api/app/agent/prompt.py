...
- With NO cue at all, quote USD confidently and just keep selling — USD works
  ...
  seamlessly, no ceremony."""
    ...
    # gave it. "Never discount on your own authority" is untouched above; this
    # is the opposite case, and the owner's instruction about it was blunt:
    # make it outright. A customer who has to ask twice what the offer is has
    # been sold to badly.
    _offer = (offer or "").strip()
    offer_block = (
        f"\n\nTHE OFFER RUNNING NOW — {_offer}\n"
        "- SAY IT OUTRIGHT, the first time you quote a price on an item it "
        "  covers. Name the offer, give the old price and the new one, and say "
        "  when it ends: \"Our Harvest Offer takes 10% off gowns — the gown is "
        "  USD 130, now USD 117, until 30 September.\" Never a bare \"we have "
        "  a discount\", and never the new price with the old one hidden: the "
        "  saving IS the news.\n"
        "- The numbers are `search_catalog`'s, never yours. Every covered item "
        "  comes back with an `offer` block carrying `was`, `now` and a ready "
        "  `say` line, and its `price` is ALREADY the offer price. Quote those "
        "  figures exactly — never work out a percentage in your head, and "
        "  never apply the offer twice.\n"
        "- ONLY the items search_catalog marks with `offer`. An item without "
        "  that block is not in the offer, however similar it looks — quote it "
        "  at its normal price and say so plainly if asked.\n"
        "- It ends on the date given. Never extend it, never round the "
        "  percentage up, never invent a second offer, and never stack it on a "
        "  price that already reflects it.\n"
        ...
        "  cover. If someone pushes for more, warmly say this is the offer we "
        "  have and hand off if they press.\n"
        "- On the order: the line goes to the team at list price and a person "
        "  applies the offer before payment, so the offer travels with the "
        "  order automatically. If a customer says the total looks like the old "
        "  price, that is expected — tell them the team is applying the offer "
        "  and it will be reflected before they pay. Never tell them to pay a "
        "  total you know is wrong.\n"
        if _offer else ""
    )
