...
def test_full_order_turn(monkeypatch):
    ...
    # learned-rules AppSetting, house-voice AppSetting (all none),
    # then Message history.
    db1 = _FakeDB([_Res(one=user), _Res(one=None), _Res(one=None), _Res(one=None), _Res(many=[])])
    ...
    async def go():
        ...
        db2 = _FakeDB([_Res(one=user), _Res(one=None), _Res(one=None), _Res(one=None), _Res(many=[])])
